...
#include "vn_device.h"
#include "vn_device_memory.h" /* helios resource identity (dcomp vehicle) */
#include "vn_image.h"
                    ...
                    struct vn_image **out_img)
{
   ...
   img->wsi.helios_external_blit_src = wsi_info->helios_external_blit_src;
   if (VN_DEBUG(WSI)) {
...
vn_QueuePresentKHR(VkQueue _queue, const VkPresentInfoKHR *pPresentInfo)
{
   ...
   /* Win32 software WSI has its own per-swapchain present worker in common
    * WSI.  A second Venus queue worker would only delay surface errors until
    * the next present and prevent current pResults from being populated.
    */
